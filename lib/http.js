...
function httpsJsonOnce(urlStr, headers, proxyUrl, timeoutMs) {
...
export async function httpsJson(urlStr, headers = {}, opts = {}) {
  ...
  try {
    return await httpsJsonOnce(urlStr, hdrs, "", timeoutMs);
  } catch (directErr) {
    const proxyUrl = opts.proxyUrl || (await resolveProxyUrl());
    if (!proxyUrl) throw directErr;
    try {
      return await httpsJsonOnce(urlStr, hdrs, proxyUrl, timeoutMs);
    } catch (proxyErr) {
