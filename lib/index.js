...
import { collectPiAiProviders, collectLlmRoutes, matchWanted, sortProviders, validProviderId, DEFAULT_ORDER } from "./detect.js";
...
async function wantedBoardIds(ctx) {
	const routes = new Set([
		...
		...collectHubLogins(readAuthStore())
	]);
	const wanted = matchWanted(routes);
...
async function buildSnapshot(ctx) {
	...
	const providers = await Promise.all(ranked.map((p) => runProvider(p, ctx)));
